...
class CategoriesController extends GetxController {
  ...
  RxBool hasLoadedMoreOnScroll = false.obs;
  ...
  void _setupScrollListener() {
    scrollController.addListener(() {
      ...
      // Load more items on first scroll attempt if showing only 4 items
      if (!hasLoadedMoreOnScroll.value && items.length == 4 && !isLoading.value) {
        hasLoadedMoreOnScroll.value = true;
      ...
      // Check if user is at bottom (only when there are more than 4 items)
      if (items.length > 4 && scrollController.position.pixels >=
  ...
  void _updateLoadMoreIconVisibility() {
    if (isScrolling.value) {
      ...
      // Show initially when 4 or fewer items
      showLoadMoreIcon.value = true;
    } else {
      // Hide in other cases
      showLoadMoreIcon.value = false;
  ...
  void loadInitialItems() {
    allItems = [
      ...
      {
        'imageUrl': AppImages.product1,
        'title': 'Premium Headphones - Wireless & Noise Cancelling',
        'isLive': true,
        'viewerCount': 87,
      ...
      {
        'imageUrl': AppImages.product2,
        'title': 'Action Figure Collection - Superhero Edition',
        'isLive': false,
        'viewerCount': 23,
        'shopName': 'Toy Universe',
        'ownerPic': AppImages.productOwner,
        'category': 'Kids',
        'keywords': ['action', 'figure', 'superhero', 'collection'],
      },
      {
        'imageUrl': AppImages.product3,
        'title': 'VR Gaming Headset - Next Gen Experience',
        'isLive': true,
        'viewerCount': 156,
        'shopName': 'VR Central',
        'ownerPic': AppImages.productOwner,
        'category': 'Video Games',
        'keywords': ['vr', 'gaming', 'headset', 'virtual reality'],
      },
      {
        'imageUrl': AppImages.product4,
      ...
      {
        'imageUrl': AppImages.product1,
        'title': 'Building Blocks Set - Creative Learning Kit',
        'isLive': false,
        'viewerCount': 45,
        'shopName': 'Learning Hub',
        'ownerPic': AppImages.productOwner,
        'category': 'Kids',
        'keywords': ['building', 'blocks', 'creative', 'learning'],
      },
      {
        'imageUrl': AppImages.product2,
    ...
    items.value = allItems.take(4).toList();
  }
  ...
  void loadMoreItems() {
    ...
    Future.delayed(const Duration(milliseconds: 800), () {
      ...
      if (searchQuery.value.isNotEmpty) {
        ...
        items.value = allItems.take(items.length + 2).toList();
      }
  ...
  void loadMoreItemsOnScroll() {
    ...
    Future.delayed(const Duration(milliseconds: 300), () {
      if (searchQuery.value.isEmpty) {
        // Load 4 more items from allItems
        int currentLength = items.length;
        int newLength = (currentLength + 4).clamp(0, allItems.length);
        items.value = allItems.take(newLength).toList();
      }
      isLoading.value = false;
    });
  }
